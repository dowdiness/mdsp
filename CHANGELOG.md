...
- **Facade re-export** — the top-level `dowdiness/moondsp` module exposes
  the full library surface via `pub using` from `@dsp`, `@graph`, and
  `@voice`, so consumers write `@moondsp.X`. A single root facade avoids
  the drift class where symbols added to one re-export were missed in
  another (notably `CompiledTemplate`).
- **Test suite** — 573 tests across DSP, graph, voice, pattern, mini,
...
- **Labelled constructor arguments**: five footgun-prone signatures
  require labels at the call site, eliminating whole classes of unit /
  ordering bugs (ms vs. s on ADSR times, sample-rate vs. block-size swap
  on `DspContext`, cutoff-vs-Q swap on biquad):
